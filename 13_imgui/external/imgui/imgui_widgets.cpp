...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    {
        // Hue is lost when converting from greyscale rgb (saturation=0). Restore it.
        ColorConvertRGBtoHSV(_R, _G, _B, _H, _S, _V);
        if (memcmp(g.ColorEditLastColor, col, sizeof(float) * 3) == 0)
        {
            if (_S == 0)
                _H = g.ColorEditLastHue;
            if (_V == 0)
                _S = g.ColorEditLastSat;
        }
    }
    else if (flags & ImGuiColorEditFlags_InputHSV)
    {
        ColorConvertHSVtoRGB(_H, _S, _V, _R, _G, _B);
    }
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive())
        {
            ...
            if (initial_dist2 >= (wheel_r_inner - 1) * (wheel_r_inner - 1) && initial_dist2 <= (wheel_r_outer + 1) * (wheel_r_outer + 1))
            {
                ...
                _H = ImAtan2(current_off.y, current_off.x) / IM_PI * 0.5f;
                if (_H < 0.0f)
                    _H += 1.0f;
                value_changed = value_changed_h = true;
            }
            float cos_hue_angle = ImCos(-_H * 2.0f * IM_PI);
            float sin_hue_angle = ImSin(-_H * 2.0f * IM_PI);
            if (ImTriangleContainsPoint(triangle_pa, triangle_pb, triangle_pc, ImRotate(initial_off, cos_hue_angle, sin_hue_angle)))
            {
                ...
                _V = ImClamp(1.0f - vv, 0.0001f, 1.0f);
                _S = ImClamp(uu / _V, 0.0001f, 1.0f);
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        if (IsItemActive())
        {
            _S = ImSaturate((io.MousePos.x - picker_pos.x) / (sv_picker_size - 1));
            _V = 1.0f - ImSaturate((io.MousePos.y - picker_pos.y) / (sv_picker_size - 1));
            value_changed = value_changed_sv = true;
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context");
        ...
        if (IsItemActive())
        {
            _H = ImSaturate((io.MousePos.y - picker_pos.y) / (sv_picker_size - 1));
            value_changed = value_changed_h = true;
        }
    }
    ...
    if (value_changed_h || value_changed_sv)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ColorConvertHSVtoRGB(_H >= 1.0f ? _H - 10 * 1e-6f : _H, _S > 0.0f ? _S : 10 * 1e-6f, _V > 0.0f ? _V : 1e-6f, col[0], col[1], col[2]);
            g.ColorEditLastHue = _H;
            g.ColorEditLastSat = _S;
    ...
    if (value_changed_fix_hue_wrap && (flags & ImGuiColorEditFlags_InputRGB))
    {
        ...
        {
            if (new_V <= 0 && _V != new_V)
                ColorConvertHSVtoRGB(_H, _S, new_V <= 0 ? _V * 0.5f : new_V, col[0], col[1], col[2]);
            else if (new_S <= 0)
                ColorConvertHSVtoRGB(_H, new_S <= 0 ? _S * 0.5f : new_S, new_V, col[0], col[1], col[2]);
        }
    }
    ...
    ImVec4 hue_color_f(1, 1, 1, style.Alpha); ColorConvertHSVtoRGB(_H, 1, 1, hue_color_f.x, hue_color_f.y, hue_color_f.z);
    ImU32 hue_color32 = ColorConvertFloat4ToU32(hue_color_f);
    ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(ImVec4(_R, _G, _B, style.Alpha)); // Important: this is still including the main rendering/style alpha!!
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        sv_cursor_pos = ImLerp(ImLerp(trc, tra, ImSaturate(_S)), trb, ImSaturate(1 - _V));
    }
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        // Render SV Square
        ...
        sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(_S)     * sv_picker_size), picker_pos.x + 2, picker_pos.x + sv_picker_size - 2); // Sneakily prevent the circle to stick out too much
        sv_cursor_pos.y = ImClamp(IM_ROUND(picker_pos.y + ImSaturate(1 - _V) * sv_picker_size), picker_pos.y + 2, picker_pos.y + sv_picker_size - 2);

        // Render Hue Bar
        for (int i = 0; i < 6; ++i)
            draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1]);
        float bar0_line_y = IM_ROUND(picker_pos.y + _H * sv_picker_size);
